...
exports.register = async (req, res, next) => {
   ...
   try {
      ...
      return res.status(201).json({ message: 'User has been created, Please login..' });
   } catch (err) {
      passError(err, next);
   }
};
...
exports.remindPassword = async (req, res, next) => {
   try {
      ...
      return res.status(200).json({ status: 'Verification link sent' });
   } catch (err) {
      passError(err, next);
   }
};
...
exports.resetPassword = async (req, res, next) => {
   try {
      ...
      return res.status(200).json({ status: 'New password has been set' });
   } catch (err) {
      passError(err, next);
   }
};
