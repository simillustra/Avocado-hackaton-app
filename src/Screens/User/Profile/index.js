    ...
    <View style={{backgroundColor: 'white'}}>
      <View style={styles.header}>
        <Pressable style={styles.icon} onPress={handleIconPress}>
          <Typography text="X" bold color="white" fontSize={20} />
        </Pressable>
      </View>
      ...
      <View style={styles.userDetails}>
        ...
        <Typography fontSize={30} text={userInfo.username} bold />
      </View>
      {navigationList.map((item, index) => (
        <TouchableOpacity activeOpacity={0.6} key={index} onPress={item.action}>
          <NavList iconName={item.icon} text={item.name} />
