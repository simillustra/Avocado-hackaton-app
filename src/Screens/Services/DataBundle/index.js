...
const DataBundle = ({route, navigation}) => {
  ...
  const getServiceList = useCallback(
    async (params) => {
      try {
        ...
        const dataPackage = await helpers.postAction(
          'http://fsi.ng/api/v1/baxipay/services/databundle/bundles',
          formData,
        );
        if (dataPackage) {
          console.log('R- dataPackage.data', dataPackage);
          ...
          if (dataPackage.code !== 200) {
            ...
            Toast.show({
              ...
              text2: dataPackage.message,
              position: 'bottom',
            });
            return;
          } else {
            setIsProcessing(false);
            navigation.navigate('DataBundleList', {
              screenData: dataPackage.data,              
              merchantInfo: userAction,
            });
          }
        }
      } catch (error) {
        console.log('Error- dataPackage.data', error);
        Toast.show({
          type: 'error',
          text1: 'Request status',
          text2: error.message,
